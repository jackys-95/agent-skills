...
- **Do not mutate indexes casually.** `qmd collection add`, `qmd update`, and
  `qmd embed` change local state and can be expensive. For task-memory-bank
  projects, route reindexing through `memory_bank.py reindex` instead.
