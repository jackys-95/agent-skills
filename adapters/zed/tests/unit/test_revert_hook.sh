...
# copies back a real snapshot, or deletes a file whose base is "new" (created this
# turn).
set -euo pipefail
...
pointer_of() { echo "/tmp/cc_zed_ptr_$(hash_of "$1")"; }

# 6a: no pointer for the file → exit 1 (nothing to revert)
missing="/tmp/zed_revert_nopointer.txt"
rm -f "$missing" "$(pointer_of "$missing")"
code=0; out=$(python3 "$HOOK" "$missing" 2>&1) || code=$?
[ $code -eq 1 ] && ok "6a: no pointer → exit 1" || fail "6a: exit=$code out='$out'"

# 6b: real snapshot → file restored to snapshot content. The pointer is keyed only
# by path (no session), matching production: revert must survive turn boundaries.
f="/tmp/zed_revert_edit.txt"
snap="/tmp/zed_revert_edit_snapshot"
ptr=$(pointer_of "$f")
...
# 6c: "new" pointer (new file created this turn) → file deleted
...
if [ $code -eq 0 ] && [ ! -e "$nf" ] && echo "$out" | grep -q 'deleted'; then
    ok "6c: 'new' pointer → new file deleted"
else
    fail "6c: exit=$code exists=$([ -e "$nf" ] && echo yes || echo no) out='$out'"
fi
rm -f "$nf" "$nfptr"

# 6d: "new" pointer but file already gone (user deleted it) → still exit 0, no crash
gone="/tmp/zed_revert_alreadygone.txt"
goneptr=$(pointer_of "$gone")
printf 'new' > "$goneptr"
rm -f "$gone"
code=0; out=$(python3 "$HOOK" "$gone" 2>&1) || code=$?
[ $code -eq 0 ] && ok "6d: 'new' pointer, file already gone → exit 0, no crash" || fail "6d: exit=$code out='$out'"
rm -f "$goneptr"
