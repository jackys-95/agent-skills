...
# `zed -a --diff ... --diff ...` multi-diff and clears the markers.
set -euo pipefail

HOOK="$(dirname "$0")/../../hooks/stop_flush_zed_diffs.py"
PRE_HOOK="$(dirname "$0")/../../hooks/pre_edit_zed_snapshot.py"
POST_HOOK="$(dirname "$0")/../../hooks/post_edit_open_in_zed.py"
PASS=0; FAIL=0
SID="stopsess"
...
seen_glob="/tmp/cc_zed_seen_${SID}_*"
rm -f $seen_glob
...
# --diff pairs; new file diffs against /dev/null; markers cleared afterward.
...
echo "changed1" > "$f1"
# f2: new file — only the post-hook ran (no pre snapshot), so base must be /dev/null
echo "created2" > "$f2"
...
# 3d: markers cleared after flush — a second Stop is a no-op
: > "$ZED_ARGS_LOG"
CC_ZED_HOOK=1 PATH="$SHIM_DIR:$PATH" python3 "$HOOK" <<< "{\"session_id\":\"$SID\"}" > /dev/null 2>&1
if [ ! -s "$ZED_ARGS_LOG" ] && ! ls $seen_glob >/dev/null 2>&1; then
    ok "3d: markers cleared → second Stop is a no-op"
else
    fail "3d: leftover markers or zed relaunched (args=$(cat "$ZED_ARGS_LOG"))"
fi

# Cleanup
f1hash=$(hash_of "$f1"); f2hash=$(hash_of "$f2")
rm -f "$f1" "$f2" $seen_glob
rm -f /tmp/cc_pre_${f1hash}_* "/tmp/cc_pre_ptr_${f1hash}" "/tmp/cc_gen_${f1hash}"
rm -rf "$SHIM_DIR"
