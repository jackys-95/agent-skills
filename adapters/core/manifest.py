...
the shared seam (see #51 for the full contract).
...
def _seed_if_new(data, namespace, session_id, path):
    """Idempotent: if `path` has no entry yet this turn, capture its base (snapshot, or
    "new" if it doesn't exist yet) and add an entry to `data` in place. No-ops if an
    entry already exists — keeps the FIRST base captured this turn, so N edits to the
    same file in one turn still diff/revert against the turn-start state.
    ...
    Caller is responsible for persisting `data`."""
...
def seed_if_new(namespace, session_id, path):
    """Load/seed/save wrapper around `_seed_if_new` for single-file callers."""
    data = _load(namespace, session_id)
    base = _seed_if_new(data, namespace, session_id, path)
    if base is not None:
...
def mark_touched(namespace, session_id, path):
    ...
    independently of capturing the turn-start base before the tool ran — that
    split is deliberate, not accidental duplication."""
    ...
    for root in roots:
        for dirpath, dirnames, filenames in os.walk(root):
            ...
            for name in filenames:
                _seed_if_new(data, namespace, session_id, os.path.join(dirpath, name))
    _save(namespace, session_id, data)
